...
// O6 skip: if the most recent decision was a Planner-accepted reconciliation,
...
export const denyMessage = (reason: string): string => {
  ...
  return `LATHE GATE BLOCKED: ${reason}. Your next tool call must be meridian-bridge_ask_planner and must state the intended file and edit, rationale, current status, and evidence. The Planner owns engineering direction; the Executor owns implementation after approval. Continue only on proceed or proceed_with_constraints. Reads remain available for gathering evidence.`;
};
