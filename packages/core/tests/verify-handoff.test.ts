...
const makeRef = (overrides?: { packet?: Packet; turnComplete?: boolean; awaitingVerification?: boolean }) => {
  ...
  const ctx = {
    ...
    config: {
      thresholds: {
        ...
        reportRejectionParkAt: 3, checkpointBounceLimit: 1,
        verificationTimeoutMs: 600000, maxPasses: 3, maxStallRetries: 2,
        maxReorientRetries: 2, maxRunMs: 6 * 60 * 60 * 1000,
      },
      opencode: { bridgePort: 0 },
      mutationCommandPatterns: ["\\b(pnpm|npm|yarn)\\b.*\\bgenerate\\b", "task contracts", "dotnet-rivet"],
      daddy: { providerId: "test", modelId: "test", agent: "test", timeoutMs: 300000, transportRetries: 3 },
    },
    paths,
    worktree: tmp,
    packet,
  ...
  store.writeMeta({ runId: packet.runId, status: "running", attempt: 1, repo: "/tmp/test-repo", base: "main", branch: "meridian/test", worktree: tmp, updatedAt: clock.nowIso() });
  store.writeLedger(store.initialLedger(packet));
  store.writeGateState(packet.runId, { runId: packet.runId, latched: false, firstEditApproved: false, reconciliationRequired: false, expectedGlobs: ["src/**"], suspiciousGlobs: [], baselineDiffStats: {}, updatedAt: clock.nowIso(), mutationCommandPatterns: [] });
  return { ref, tmp, clock };
};

const makeHandoffArtifact = (overrides?: Partial<{ runId: string; completedSteps: { description: string; files?: string[] }[]; remainingWork: string[]; decisionsMade: string[]; resumeFrom: string }>) => ({
...
test("verify_handoff: returns verdict and clears awaitingVerification", async () => {
  ...
  await (async () => {
    ...
    await wf(join(handoffDir, "handoff.ts"), "import { z } from 'zod';\nexport const Foo = z.string();", "utf-8");
  })();
  ...
  const verdictJson = JSON.stringify({ ok: true, trusted: [{ description: "added handoff.ts", files: ["src/domain/handoff.ts"] }], issues: [], resumeHint: "add tests" });
  let capturedPrompt = "";
...
test("verify_handoff: prompt includes all claimed steps, diff, file samples, and questions", async () => {
  const { ref, tmp } = makeRef();
  const runId = ref.current.packet.runId;
  const paths = makePaths(tmp);
  const runDir = paths.runDir(runId);
  ...
  await (async () => {
    const { mkdir, writeFile: wf } = await import("node:fs/promises");
    await mkdir(handoffDir, { recursive: true });
    await wf(join(handoffDir, "handoff.ts"), "import { z } from 'zod';\nexport const Foo = z.string();\nexport const Bar = z.number();", "utf-8");
  })();
  ...
  ref.current.executor = {
    ...
    sendMessage: async (_s, text: string) => { capturedPrompt = text; return { info: { tokens: {} }, parts: [{ type: "text", text: verdictJson }] }; },
    listMessages: async () => [],
    deleteSession: async () => {},
  } as any;
  ...
  ok(capturedPrompt.includes("## Claimed completions"), "prompt should include claimed completions section");
...
test("verify_handoff: valid verdict JSON passed through correctly", async () => {
  ...
  const verdictJson = JSON.stringify({
    ...
    issues: [{ file: "src/bar.ts", problem: "missing export" }, { file: "src/baz.ts", problem: "type error" }],
    resumeHint: "fix the type error in baz before continuing",
  });

  ref.current.executor = {
    createSession: async () => "s",
    sendMessage: async () => ({ info: { tokens: {} }, parts: [{ type: "text", text: verdictJson }] }),
    listMessages: async () => [],
    deleteSession: async () => {},
  } as any;
...
test("verify_handoff: unparseable daddy response returns fallback verdict", async () => {
  ...
  ref.current.executor = {
    createSession: async () => "s",
    sendMessage: async () => ({ info: { tokens: {} }, parts: [{ type: "text", text: "I'm not sure, let me think about this... maybe { ok: true }" }] }),
    listMessages: async () => [],
    deleteSession: async () => {},
  } as any;
...
test("verify_handoff: executor exception returns error verdict", async () => {
  ...
  ref.current.executor = {
    createSession: async () => "s",
    sendMessage: async () => { throw new Error("provider timeout"); },
    listMessages: async () => [],
    deleteSession: async () => {},
  } as any;
...
test("runVerify: creates session, sends prompt, returns parsed verdict", async () => {
  const executor: Executor = {
    createSession: async (title, dir) => { strictEqual(title, "meridian-verify"); return "session-123"; },
    sendMessage: async (sessionId, text, model, timeout) => {
      ...
      return { info: { tokens: {} }, parts: [{ type: "text", text: JSON.stringify({ ok: true, trusted: [], issues: [], resumeHint: "go" }) }] };
    },
    listMessages: async () => [],
    deleteSession: async (sessionId) => { strictEqual(sessionId, "session-123"); },
  };
...
test("runVerify: executor failure returns error verdict", async () => {
  const executor: Executor = {
    createSession: async () => { throw new Error("network"); },
...
test("write_handoff: returns error when no active run", async () => {
  const result = await handleWriteHandoff({ current: null }, {
    completedSteps: [{ description: "step" }],
    remainingWork: [],
    decisionsMade: [],
    resumeFrom: "now",
