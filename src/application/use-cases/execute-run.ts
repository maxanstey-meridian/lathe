...
import { q1InitialSeed, q2RotationSeed, q8ReconciliationSeed, renderDaddySeed } from "../../domain/prompts.js"

import { turnLoop } from "./turn-loop.js"
import { journal, type RunPorts, type RunChannel, type Seed, type TurnLoopResult } from "./run-runtime.js"
...
export const makeExecuteRun = <Ref>(ports: RunPorts, bridge: BridgeBinding<Ref>): ExecuteRunCallback<Ref> =>
  async (runId, runMeta, ref): Promise<void> => {
    ...
    const daddySessionId = isResume && priorMeta?.daddySessionId
    ...
    store.writeActiveRun({ runId, runDir: dirname(worktree), worktree, babySessionId, startedAt: clock.nowIso() })
    ...
    if (!isResume) {
      ...
      if (checkpoint) {
        seed = {
          name: "Q2",
          text: q2RotationSeed(packet, store.readLedger(runId), checkpoint, store.readReviewState(runId), store.readDecisions(runId), repo.diffStat(worktree, base)),
        }
      } else {
        const gate = store.readGateState(runId)
        store.writeGateState(runId, {
        ...
        seed = {
          name: "Q8",
          text: q8ReconciliationSeed(packet, store.readLedger(runId), store.readReviewState(runId), store.readDecisions(runId), repo.diffStat(worktree, base)),
        }
      }
    }
...
const finalizeRun = (ports: RunPorts, runId: string, worktree: string, result: TurnLoopResult): void => {
  const { store, repo, clock } = ports
  const { outcome } = result

  const sha = repo.wipCommit(worktree, `meridian: WIP ${runId} [${outcome.status}]`)
  if (sha) journal(ports, runId, 0, { event: "committed", sha, message: `meridian: WIP ${runId} [${outcome.status}]` })
  ...
  if (outcome.status === "blocked") {
    journal(ports, runId, 0, { event: "parked", reason: outcome.reason, question: outcome.question })
    store.writeMeta({ ...meta, status: "blocked", blockedReason: outcome.reason, blockedQuestion: outcome.question, endedAt: clock.nowIso(), updatedAt: clock.nowIso() })
  } else {
    if (outcome.status === "failed") journal(ports, runId, 0, { event: "driver_note", note: outcome.note })
    store.writeMeta({ ...meta, status: outcome.status, endedAt: clock.nowIso(), updatedAt: clock.nowIso() })
  }
