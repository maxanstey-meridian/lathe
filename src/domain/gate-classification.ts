...
export const classifyChangedFiles = (
  ...
  { path: string } & {
    classification: "expected" | "suspicious" | "acceptable-but-not-predeclared";
    reason: string;
    action: "kept";
  }
> => {
    ...
    .map((path) => {
      if (expected.some(({ re }) => re.test(path))) {
        return {
          ...
          reason: "in the declared change surface",
      ...
      if (suspicious.some(({ re }) => re.test(path))) {
        return {
          ...
          reason: "in the suspicious surface — flagged for review",
          action: "kept" as const,
        };
      }
      return {
