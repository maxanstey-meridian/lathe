...
const checkpointProblems = (checkpoint: Checkpoint, packet: Packet, ledger: OutcomeLedger): string[] => {
  const problems: string[] = []
...
const text = (t: string) => ({ content: [{ type: "text" as const, text: t }], isError: false as const })
...
export const handleUpdateOutcomes = async (ref: RunRef, input: UpdateOutcomesInput) => {
  ...
  for (const update of input.outcomes) {
    ...
    const evidenceIsBlank = !update.evidence || update.evidence.length === 0 || update.evidence.every((e) => !e.trim())
...
export const handleGetDecisions = async (ref: RunRef, input: GetDecisionsInput) => {
  ...
  const accepted = decisions
    ...
    .find((d) => ACCEPTED_STATUSES.some((s) => s === d.status) && (gateState.lastAcceptedDecisionAt ?? "") < d.timestamp)
