...
import { parsePacketShape, stampBase, extractRepoFromYaml, extractBaseFromYaml } from "../domain/packet.js"
...
export class StoreAdapter implements Store {
  ...
  listQueue(): QueueEntry[] {
    ...
    const requeued: QueueEntry[] = this.listRunIds()
      .flatMap((runId) => {
  ...
  admitQueue(runId: string, raw: string): void {
    ...
    if (!baseInFm) {
      try {
        ...
        problems.push("headBranch failed: repo is not a valid git repository or is in a detached HEAD state")
        this.archiveAndFail(runId, raw, problems)
        return
      }
    }
