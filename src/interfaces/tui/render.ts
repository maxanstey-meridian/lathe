...
export const renderStatus = (store: Store): string => {
  ...
  if (active) {
    ...
    for (const e of store.readJournal(active.runId).slice(-5)) lines.push(`  ${e.at.slice(11, 19)} ${e.event}`)
  } else {
  ...
  for (const m of parked) {
    if (!m) continue
    const retries = m.stallRetries ? `, ${m.stallRetries} auto-retr${m.stallRetries === 1 ? "y" : "ies"}` : ""
    lines.push(`parked: ${m.runId} (${m.blockedReason ?? "?"}${retries}) — ${(m.blockedQuestion ?? "").slice(0, 100)}`)
  }
  ...
  if (campaigns.length > 0) {
    ...
    for (const c of campaigns) {
      ...
      lines.push(`  ${mark} ${c.campaignId}  [${c.status}]  pass ${last?.pass ?? 0}/${c.maxPasses}  — ${c.originalIntent.slice(0, 60)}`)
    }
  }
...
export const renderReview = (store: Store): string => {
  ...
  for (const m of runs) {
    ...
    lines.push(`${icon} ${m.runId}  [${m.status}]  ${fmtOutcomes(store, m.runId)}  branch ${m.branch}`)
    if (m.status === "blocked") {
    ...
    if (m.status === "ready_for_review") {
      ...
      lines.push(`   accept: meridian accept ${m.runId} [branch]   (merges into [branch], default ${m.base}; tidies the worktree)`)
    }
  }
  return lines.join("\n")
}
