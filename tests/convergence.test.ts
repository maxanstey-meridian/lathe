...
test("parsePlannerResponse: fenced JSON, bare JSON, garbage → stop", () => {
  const good = {
    status: "proceed", answer: "a", constraints: [], evidence_used: [],
    safe_next_action: "x", human_decision_needed: null,
  }
  ...
  assert.throws(() =>
    renderFollowupPacket({
      original: {
        ...
        frontmatter: {
          repo: "/tmp/x", base: "main",
          outcomes: [{ id: "a", description: "a" }],
          expected_surface: ["src/**"], verification: [{ command: "true" }], constraints: [], pass: 1, regression_outcomes: [],
        },
        body: "", raw: "",
      },
