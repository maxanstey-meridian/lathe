...
const fakePlanner = (): Planner => ({
  ...
  consult: async () => ({ status: "proceed", answer: "go", constraints: [], evidence_used: [], safe_next_action: "x", human_decision_needed: null }),
  finalReview: async () => ({ verdict: "accept", findings: [], notes: "ok", human_decision_needed: null }),
})
...
const scriptedExecutor = (channel: RunChannel, steps: Array<{ intents?: BridgeIntent[] }>, newSessionIds: string[] = []): Executor => {
  let i = 0
  let s = 0
  return {
...
test("rotateSession: replaces the session, updates meta, latches first-edit (with checkpoint)", () => {
  return (async () => {
    ...
    store.writeMeta({
      runId: RUN_ID, status: "running", attempt: 1, repo: "/tmp/test-repo", base: "main",
      branch: `meridian/${RUN_ID}`, worktree: "/tmp/wt", babySessionId: "baby-0", stallRetries: 0, reorientRetries: 0, updatedAt: "2026-01-01T00:00:00.000Z",
    })
    store.writeGateState(RUN_ID, initialGateState(RUN_ID, ["src/index.ts"], [], { checkpointNudgeMs: 1, checkpointToolCalls: 1, checkpointFiles: 1, checkpointLoc: 1, mutationCommandPatterns: [] }, "2026-01-01T00:00:00.000Z"))
...
test("rotateSession: no checkpoint stacks reconciliation (O6)", () => {
  return (async () => {
    ...
    const ports = makePorts(store, fakeRepo(), scriptedExecutor(emptyChannel(), [], ["baby-1"]), fakePlanner())
...
test("makeExecuteRun: fresh run → freeze + init state → terminal status in meta", () => {
  return (async () => {
    ...
    const executor = scriptedExecutor(channel, [
      { intents: [{ kind: "report-accepted", status: "blocked", blockedReason: "human_decision", blockedQuestion: "decide", summary: "blocked out" }] },
    ])
    const ports = makePorts(store, fakeRepo(), executor, fakePlanner())
    const bridge: BridgeBinding<unknown> = { beginRun: () => channel, endRun: () => {} }

    const executeRun = makeExecuteRun(ports, bridge)
    await executeRun(RUN_ID, { repo: "/tmp/test-repo", worktree: join(tmp, "wt"), base: "main", branch: `meridian/${RUN_ID}` }, {}, ports.clock)
...
test("makeExecuteRun: real fresh queue path — reads from queue dir, not pre-frozen", () => {
  return (async () => {
    ...
    const executeRun = makeExecuteRun(ports, bridge)
    await executeRun(RUN_ID, { repo: "/tmp/test-repo", worktree: join(tmp, "runs", RUN_ID, "worktree"), base: "main", branch: `meridian/${RUN_ID}` }, {}, ports.clock)
    ...
    ok(store.readFrozenPacket(RUN_ID).includes("---"), "frozen packet was written from queue source")
    // The ledger + gate were initialised on the fresh attempt.
    equal(store.readLedger(RUN_ID).outcomes.length, 1)
    ok(store.readGateState(RUN_ID))
...
test("makeExecuteRun: resume → reuses prior Daddy session ID, refreshes gate", () => {
  return (async () => {
    ...
    store.writeMeta({
      runId: RUN_ID, status: "queued", attempt: 1, repo: "/tmp/test-repo", base: "main",
      branch: `meridian/${RUN_ID}`, worktree: join(tmp, "runs", RUN_ID, "worktree"),
      babySessionId: "baby-old", daddySessionId: "daddy-prior",
      stallRetries: 0, reorientRetries: 2, updatedAt: "2026-01-01T00:00:00.000Z",
    })
    // Prior gate state.
    store.writeGateState(RUN_ID, initialGateState(RUN_ID, ["src/index.ts"], [], {
      checkpointNudgeMs: 1_000_000, checkpointToolCalls: 50, checkpointFiles: 6,
      checkpointLoc: 80, mutationCommandPatterns: [],
    ...
    const resumePlanner: Planner = {
      handshake: async () => { handshakeCalled = true; return "daddy-new" },
    ...
    const channel = emptyChannel()
    const executor = scriptedExecutor(channel, [
      { intents: [{ kind: "report-accepted", status: "blocked", blockedReason: "stop_condition", blockedQuestion: "done", summary: "resumed" }] },
    ])
    ...
    equal(handshakeCalled, false, "handshake NOT called on resume — resumeSession was used instead")
    // Gate was refreshed with current config thresholds.
    const gate = store.readGateState(RUN_ID)
    equal(gate.checkpointNudgeMs, 1200000)
    deepEqual(gate.mutationCommandPatterns, ["\\b(pnpm|npm|yarn)\\b.*\\bgenerate\\b", "task contracts", "dotnet-rivet"])
...
test("makeExecuteRun: invalid frozen packet → meta failed, no throw", () => {
  return (async () => {
    ...
    store.writeMeta({
      runId: RUN_ID, status: "running", attempt: 1, repo: "/tmp/test-repo", base: "main",
      branch: `meridian/${RUN_ID}`, worktree: join(tmp, "wt"), stallRetries: 0, reorientRetries: 0, updatedAt: "2026-01-01T00:00:00.000Z",
    })
