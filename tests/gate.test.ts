...
test("isMutation: bash tools with mutation verbs or patterns", () => {
  ...
  // Custom patterns
  assert.ok(isMutation("bash", { command: "pnpm generate" }, ["\\b(pnpm|npm|yarn)\\b.*\\bgenerate\\b"]))
...
test("gateTriggerReason: first edit approved, no reconciliation → no trigger (G5: cadence gone)", () => {
  ...
  const reason = gateTriggerReason(state, { files: ["src/file.ts", "other.txt", "deep/nested.go"], loc: 0 })
...
test("rotationGateState: clean rotation (hasCheckpoint) re-latches first-edit, no reconciliation", () => {
  ...
  assert.strictEqual(result.next.firstEditApproved, false, "replaced session must re-earn first-edit")
  assert.strictEqual(result.next.latched, true)
  assert.strictEqual(result.next.reconciliationRequired, false, "valid checkpoint needs no reconciliation")
...
test("mutationDenyReason: out-of-surface absolute path wins first", () => {
  const state = makeState({ latched: false, firstEditApproved: false, reconciliationRequired: true })
  const reason = mutationDenyReason("edit", { filePath: "/etc/passwd" }, state, "/home/wt", undefined)
...
test("mutationDenyReason: when latched, latch reason is returned", () => {
  ...
  const reason = mutationDenyReason("edit", { filePath: "src/file.ts" }, state, "/home/wt", undefined)
  assert.strictEqual(reason, "initial latch")
})

test("mutationDenyReason: memory latch fires when gate is clear", () => {
  const state = makeState({ latched: false, firstEditApproved: true, reconciliationRequired: false })
  const reason = mutationDenyReason("edit", { filePath: "src/file.ts" }, state, "/home/wt", "subagent used")
  assert.strictEqual(reason, "subagent used")
})

test("mutationDenyReason: first edit unapproved (gate clear, no memory latch)", () => {
  const state = makeState({ latched: false, firstEditApproved: false, reconciliationRequired: false })
  const reason = mutationDenyReason("edit", { filePath: "src/file.ts" }, state, "/home/wt", undefined)
  assert.ok(reason?.includes("first edit"))
})

test("mutationDenyReason: reconciliation required (gate clear)", () => {
  const state = makeState({ latched: false, firstEditApproved: true, reconciliationRequired: true })
  const reason = mutationDenyReason("edit", { filePath: "src/file.ts" }, state, "/home/wt", undefined)
  assert.ok(reason?.includes("reconciliation"))
})
...
test("checkpointNudgeNotice: uses default 20 min if checkpointNudgeMs not set", () => {
  const state = makeState({ firstEditApproved: true, lastAcceptedDecisionAt: iso(25 * 60 * 1000), checkpointNudgeMs: undefined })
  const notice = checkpointNudgeNotice(state, now)
  assert.ok(notice?.includes("MERIDIAN GATE NOTICE"))
})
...
test("volumeCheckpointReason: tool-call axis", () => {
  const tc = volumeCheckpointReason(50, { files: [], loc: 0 }, { checkpointToolCalls: 50, checkpointFiles: 6, checkpointLoc: 80 })
  assert.ok(tc?.includes("50 tool calls"), `expected "50 tool calls", got: ${tc}`)
})

test("volumeCheckpointReason: files axis", () => {
  const f = volumeCheckpointReason(3, { files: ["a", "b", "c", "d", "e", "f"], loc: 10 }, { checkpointToolCalls: 50, checkpointFiles: 6, checkpointLoc: 80 })
  assert.ok(f?.includes("6 files"), `expected "6 files", got: ${f}`)
})

test("volumeCheckpointReason: LoC axis", () => {
  const l = volumeCheckpointReason(3, { files: ["a"], loc: 80 }, { checkpointToolCalls: 50, checkpointFiles: 6, checkpointLoc: 80 })
...
test("volumeCheckpointReason: tool-call axis wins when both cross", () => {
  const both = volumeCheckpointReason(50, { files: ["a", "b", "c", "d", "e", "f"], loc: 99 }, { checkpointToolCalls: 50, checkpointFiles: 6, checkpointLoc: 80 })
...
test("volumeNoticeReason: files/LoC only on mutation calls", () => {
  ...
  assert.strictEqual(volumeNoticeReason(state, 3, false, { files: ["a", "b", "c", "d", "e", "f"], loc: 90 }), undefined)
})

test("volumeNoticeReason: files/LoC on mutation calls", () => {
  const state = makeState({ checkpointToolCalls: 50, checkpointFiles: 6, checkpointLoc: 80 })
  const notice = volumeNoticeReason(state, 3, true, { files: ["a", "b", "c", "d", "e", "f"], loc: 90 })
