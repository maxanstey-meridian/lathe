...
test("removeSandbox: deletes only a real <runsDir>/<id>/worktree sandbox; refuses everything else", () => {
  ...
  try {
    ...
    assert.throws(() => removeSandbox(join(runsDir, "20990101-000000-x"), runsDir), /refusing to delete/)
...
test("diffStat: shows diff against base, not HEAD", () => {
  ...
  try {
    ...
    assert.ok(stat3.includes("b.txt") && stat3.includes("c.txt"), "should show both committed and uncommitted")
  } finally {
    rmSync(tmp, { recursive: true, force: true })
  }
})
...
test("readDiffStats: tracked changes + untracked text files, excluding node_modules and binary", () => {
  ...
  try {
    ...
    assert.strictEqual(stats["new.ts"]?.added, 3, "new.ts should have 3 segments (split on \\n with trailing)")
