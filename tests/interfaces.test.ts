...
const makeHarness = async (): Promise<Harness> => {
  ...
  store.writeLedger({
    runId: RUN_ID,
    outcomes: [{ id: "o1", description: "do it", status: "done", evidence: ["did"], updatedAt: clock.nowIso() }],
    updatedAt: clock.nowIso(),
  })
  store.appendJournal(RUN_ID, { at: clock.nowIso(), event: "run_started", runId: RUN_ID, attempt: 1 })
  store.writeCampaign({
    campaignId: RUN_ID,
    ...
    status: "converged",
    maxPasses: 3,
    passes: [{ runId: RUN_ID, pass: 1, verdict: "accept", groundedBlockers: 0, atIso: clock.nowIso() }],
    updatedAt: clock.nowIso(),
  })

  const calls = { ran: false, converged: [] as string[], superReviewed: [] as string[], planned: false, tailed: [] as string[] }
  const deps: CliDeps = {
    config: Config.parse({}),
    paths,
    store,
...
test("dispatch: successful answer requeues a parked run", async () => {
  ...
  h.deps.store.writeGateState(
    parked,
    initialGateState(parked, ["src/**"], [], { checkpointNudgeMs: 1, checkpointToolCalls: 1, checkpointFiles: 1, checkpointLoc: 1, mutationCommandPatterns: [] }, h.deps.clock.nowIso()),
  )
