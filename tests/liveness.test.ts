...
test("decideStallRecovery: wedged park under the cap → requeue, count incremented", () => {
  assert.deepEqual(decideStallRecovery({ status: "blocked", blockedReason: "wedged", stallRetries: 0 }, 2), {
    action: "requeue",
    stallRetries: 1,
  })
  assert.deepEqual(decideStallRecovery({ status: "blocked", blockedReason: "wedged", stallRetries: 1 }, 2), {
    action: "requeue",
    stallRetries: 2,
  })
})
...
test("decideStallRecovery: wedged park at the cap → escalate", () => {
  assert.deepEqual(decideStallRecovery({ status: "blocked", blockedReason: "wedged", stallRetries: 2 }, 2), {
    action: "escalate",
    stallRetries: 2,
  })
})
...
test("decideStallRecovery: non-blocked statuses are never touched", () => {
  ...
  assert.deepEqual(decideStallRecovery({ status: "queued", stallRetries: 0 }, 2), { action: "none" })
})
