...
const makeTestConfig = (overrides: Partial<Config> = {}): Config => ({
  ...
  opencode: { binary: "opencode", port: 4196, bridgePort: 4197, expectedVersion: "1.17", ...overrides.opencode },
  daddy: { providerId: "zai-coding-plan", modelId: "glm-5.1", agent: "daddy", timeoutMs: 300_000, turnSteps: 8, ...overrides.daddy },
  baby: {
    providerId: "omlx",
...
test("config generation: compaction.auto is false", () => {
  ...
  mkdirSync(join(tmpDir, "xdg", "opencode", "node_modules", "@opencode-ai", "plugin"), { recursive: true })
...
test("config generation: three custom agents with correct tool sets", () => {
  ...
  for (const tool of bridgeTools) {
    assert.strictEqual(writtenConfig.agent.daddy.tools[tool], false, `${tool} must be disabled for daddy`)
  }
  ...
  for (const tool of bridgeTools) {
    assert.strictEqual(writtenConfig.agent.superdaddy.tools[tool], false, `${tool} must be disabled for superdaddy`)
  }
})
...
test("config generation: refuses to start when plugin file is missing", () => {
  ...
  assert.throws(
    () => writeOpencodeConfig(config, paths, missingPlugin),
    /gate plugin not found/,
  )
})
...
test("config generation: seeds node_modules trio from global dir when missing", () => {
  ...
  writeFileSync(join(tempGlobalDir, "package.json"), JSON.stringify({ name: "global-opencode", version: "1.0.0" }))
  ...
  assert.ok(existsSync(join(tmpDir, "node_modules", "@opencode-ai", "plugin")), "node_modules/@opencode-ai/plugin should be copied")
