...
import type { Packet, OutcomeLedger, Checkpoint, ReviewState, Decision, PlannerResponse, SubmitReport } from "../src/domain/index.js"
...
const minLedger = (runId = "20260618-070000-test"): OutcomeLedger =>
  ({
    runId,
    outcomes: [
      { id: "test-outcome", description: "a test outcome", status: "not_started", evidence: [], updatedAt: "2026-06-18T00:00:00.000Z" },
    ],
...
describe("prompts — Q-table renderers", () => {
  ...
  describe("q2RotationSeed", () => {
    it("includes checkpoint, diff, and status line", () => {
      const ledger = { ...minLedger(), outcomes: [{ ...minLedger().outcomes[0], status: "in_progress" as const, state: "mid-impl", nextAction: "next step" }] }
      const prompt = q2RotationSeed(minPacket(), ledger, minCheckpoint(), minReview(), minDecisions(), "(clean)")
  ...
  describe("q8ReconciliationSeed", () => {
    it("says no valid checkpoint and requires reconciliation", () => {
      const prompt = q8ReconciliationSeed(minPacket(), minLedger(), minReview(), minDecisions(), "(clean)")
  ...
  describe("qReorientSeed", () => {
    it("injects planner.answer and planner.safe_next_action", () => {
      const p = minPlanner()
      const prompt = qReorientSeed(minPacket(), minLedger(), minReview(), minDecisions(), "(clean)", p)
  ...
  describe("qPlannerDecision", () => {
    it("embeds JSON.stringify({ planner }) of the argument", () => {
      const p = { status: "proceed" as const, answer: "go", constraints: [] as string[], evidence_used: [], safe_next_action: "do it", human_decision_needed: null }
    ...
    it("includes revise_slice directive when applicable", () => {
      const p = { status: "revise_slice" as const, answer: "too broad", constraints: ["narrow scope"], evidence_used: [], safe_next_action: "", human_decision_needed: null }
      const prompt = qPlannerDecision(p as PlannerResponse)
  ...
  describe("renderPlannerQuestion", () => {
    it("includes all required sections", () => {
      const prompt = renderPlannerQuestion("repo_procedure", "slice 1", "what?", "plan", ["evidence"], minReview())
