...
const makeMockExecutor = (createSessionCb?: (title: string, dir: string) => Promise<string>, deleteSessionCb?: (sessionId: string) => Promise<void>): {
  executor: Executor
  ...
  const executor: Executor = {
    ...
    sendMessage: async () => ({
      ...
      parts: [{ type: "text" as const, text: '{"verdict":"accept","findings":[],"convergence":{"recommend_stop":true,"profile":{"p0":0,"p1":0,"p2":0,"p3":0},"rationale":"ok"},"notes":"","human_decision_needed":null}' }],
    }),
    listMessages: async () => [],
