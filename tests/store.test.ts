...
const fakeRepo = (opts?: { headBranch?: string; branchExists?: boolean; headBranchThrows?: boolean; repoValid?: boolean }): Repo => {
  let headBranchCallCount = 0
  return {
    get headBranchCallCount() { return headBranchCallCount },
...
body
`
  const fm = { repo: "/tmp/test-repo", base: "main", summary: "test packet", outcomes: [{ id: "test-outcome", description: "A test outcome" }], expected_surface: ["src/index.ts"], verification: [{ command: "echo ok" }], constraints: [], ...override }
  return { runId: "20260101-000000-test", frontmatter: fm as any, body: "body\n", raw }
}

const makeValidConvergenceEntry = (overrides?: Partial<ConvergenceLogEntry>): ConvergenceLogEntry => {
...
test("store: listRunIds returns sorted ids", async () => {
  ...
  const meta1 = { runId: "20260101-000000-z", status: "queued" as const, attempt: 1, repo: "/tmp/r", base: "main", branch: "b", worktree: tmp, updatedAt: clock.nowIso() }
  const meta2 = { runId: "20260101-000000-a", status: "queued" as const, attempt: 1, repo: "/tmp/r", base: "main", branch: "b", worktree: tmp, updatedAt: clock.nowIso() }
...
test("store: ledger round-trip", async () => {
  ...
  const ledger = {
    runId: "20260101-000000-test",
    outcomes: [{ id: "test-outcome", description: "A test outcome", status: "done" as const, evidence: ["evidence.txt"], updatedAt: clock.nowIso() }],
    updatedAt: clock.nowIso(),
  }
...
test("store: decisions append and read", async () => {
  ...
  const clock = fixedClock()
  const store = StoreAdapter.create(makePaths(tmp), fakeRepo(), clock)
  const d1 = { timestamp: "2026-01-01T00:00:00.000Z", source: "daddy" as const, questionType: "other", question: "q1", status: "proceed", answer: "a1", constraints: [] }
  const d2 = { timestamp: "2026-01-01T00:00:01.000Z", source: "max" as const, questionType: "other", question: "q2", status: "proceed", answer: "a2", constraints: [] }
...
test("store: checkpoints", async () => {
  ...
  const c1 = { number: 1, reason: "checkpoint", summary: "s1", outcomes: [{ id: "o1", status: "done" as const, evidence: [] }], writtenAt: clock.nowIso() }
  const c2 = { number: 2, reason: "checkpoint", summary: "s2", outcomes: [{ id: "o1", status: "done" as const, evidence: [] }], writtenAt: clock.nowIso() }
...
test("store: convergence rejects bad schema", async () => {
  ...
  const badEntry = makeValidConvergenceEntry({ decision: { action: "author" as any, blockers: [] } })
  ...
  await rejects(async () => store.appendConvergence("20260101-000000-test", reallyBad), { message: /refusing to append/ })
  await cleanTemp(tmp)
})
...
test("store: active run lifecycle", async () => {
  ...
  const run = { runId: "20260101-000000-test", runDir: join(tmp, "runs/20260101-000000-test"), worktree: join(tmp, "worktree"), babySessionId: "sess1", startedAt: clock.nowIso() }
...
test("store: listQueue requeued runs come before fresh", async () => {
  ...
  const meta = { runId, status: "queued" as const, attempt: 1, repo: "/tmp/r", base: "main", branch: "b", worktree: join(tmp, "w"), updatedAt: clock.nowIso() }
  store.writeMeta(meta)
  await mkdir(join(tmp, "queue"), { recursive: true })
...
`
  ...
  const problems = readFileSync(join(tmp, "rejected", "20260101-000000-test.md.problems.txt"), "utf-8")
...
test("store: journal append and read", async () => {
  ...
  const clock = fixedClock()
  const store = StoreAdapter.create(makePaths(tmp), fakeRepo(), clock)
  const event = { at: clock.nowIso(), event: "run_started" as const, runId: "20260101-000000-test", attempt: 1 }
...
test("store: journal skips invalid lines (J3)", async () => {
  ...
  const valid1 = JSON.stringify({ at: "2026-01-01T00:00:00.000Z", event: "run_started", runId: "20260101-000000-test", attempt: 1 }) + "\n"
  const badLine = "this is not json\n"
  const valid2 = JSON.stringify({ at: "2026-01-01T00:00:01.000Z", event: "turn_ended", messageId: "m1", tokens: { input: 100, output: 50, reasoning: 0, cacheRead: 0, cacheWrite: 0 }, contextTokens: 150, text: "hello" }) + "\n"
