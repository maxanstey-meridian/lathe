...
const ACCEPT_REVIEW: FinalReview = { verdict: "accept", findings: [], notes: "looks good", human_decision_needed: null }
...
const toolPart = (tool: string): MessagePart => ({
  ...
  tool,
  callID: "c1",
  state: { status: "completed", input: { filePath: "src/index.ts" }, output: "ok", metadata: { exit: 0 } },
})

const scriptedExecutor = (channel: RunChannel, steps: ScriptStep[], newSessionIds: string[] = []): Executor => {
  let i = 0
  let s = 0
  return {
    ...
    sendMessage: async () => {
      ...
      if (step.pendingFinalReview !== undefined) channel.pendingFinalReview = step.pendingFinalReview
...
const seedRun = (store: Store, packet: Packet, gateDiff: Record<string, { added: number; removed: number }> = {}): void => {
  store.writeMeta({
    runId: RUN_ID,
    status: "running",
    attempt: 1,
  ...
  store.writeGateState(
    RUN_ID,
    initialGateState(RUN_ID, packet.frontmatter.expected_surface, packet.frontmatter.suspicious_surface, {
...
const makePorts = (store: Store, repo: Repo, executor: Executor, planner: Planner, config = Config.parse({})): RunPorts => ({
  config,
  store,
  repo,
  executor,
...
test("turnLoop: report → final review accept → ready_for_review with render payload", () => {
  return (async () => {
    ...
    const result = await turnLoop(ports, packet, "/tmp/worktree", "baby-0", channel, { name: "Q1", text: "go" }, FAR_FUTURE)
  ...
  return (async () => {
    ...
    const planner = fakePlanner({
      finalReview: async () => {
        calls += 1
        return calls === 1
          ? { verdict: "request_changes", findings: ["tighten the test"], notes: "", human_decision_needed: null }
          : ACCEPT_REVIEW
      },
    })
    const executor = scriptedExecutor(channel, [
...
test("turnLoop: consult round-trip — accepted decision clears the gate", () => {
  return (async () => {
    ...
    const store = StoreAdapter.create(makePaths(tmp), fakeRepo({ "src/index.ts": { added: 3, removed: 0 } }), fixedClock())
    const packet = parseFixture()
    seedRun(store, packet)
    // Latch the gate (first edit pending) so the accepted consult must clear it.
    store.writeGateState(RUN_ID, { ...store.readGateState(RUN_ID), latched: true, latchReason: "first edit of the run requires an accepted planner decision" })

    const channel = emptyChannel()
    const submission: AskPlannerInput = { questionType: "architecture_discoverable", currentSlice: "the loop", question: "shape?", approach: "gather→evaluate→execute", evidence: ["src/index.ts"] }
    const executor = scriptedExecutor(channel, [
      ...
      { intents: [{ kind: "report-accepted", status: "blocked", blockedReason: "stop_condition", blockedQuestion: "need Max", summary: "blocked" }] },
    ])
    const ports = makePorts(store, fakeRepo({ "src/index.ts": { added: 3, removed: 0 } }), executor, fakePlanner())
...
test("turnLoop: consult stop verdict → parks blocked (stop_condition)", () => {
  return (async () => {
    ...
    const channel = emptyChannel()
    const submission: AskPlannerInput = { questionType: "stop_condition", currentSlice: "x", question: "should I?", approach: "a", evidence: ["e"] }
    const stop: PlannerResponse = { status: "stop", answer: "do not proceed", constraints: [], evidence_used: [], safe_next_action: "halt", human_decision_needed: "Max must decide" }
    const executor = scriptedExecutor(channel, [{ intents: [{ kind: "consult-requested" }], pendingConsult: submission }])
    const ports = makePorts(store, fakeRepo(), executor, fakePlanner({ consult: async () => stop }))
...
test("turnLoop: gate trigger at turn end → latch + demand checkpoint (Q4)", () => {
  return (async () => {
    ...
    const executor = scriptedExecutor(channel, [
      ...
      { intents: [{ kind: "report-accepted", status: "blocked", blockedReason: "human_decision", blockedQuestion: "?", summary: "blocked" }] },
    ])
...
test("turnLoop: past the deadline → parks wedged (watchdog)", () => {
  return (async () => {
    ...
    const result = await turnLoop(ports, packet, "/tmp/worktree", "baby-0", channel, { name: "Q1", text: "go" }, 0)
...
test("turnLoop: consult reorient → session replaced, Q9 reseed, then terminal", () => {
  return (async () => {
    ...
    const channel = emptyChannel()
    const submission: AskPlannerInput = { questionType: "other", currentSlice: "reorient test", question: "should I?", approach: "a", evidence: ["e"] }
    const reorient: PlannerResponse = { status: "reorient", answer: "you're drifting", constraints: ["stick to the slice"], evidence_used: [], safe_next_action: "implement the slice", human_decision_needed: null }
    const planner = fakePlanner({ consult: async () => reorient })
    const executor = scriptedExecutor(channel, [
      { intents: [{ kind: "consult-requested" }], pendingConsult: submission },
      { intents: [{ kind: "report-accepted", status: "failed", summary: "after reorient" }] },
    ])
    const ports = makePorts(store, fakeRepo({ "src/index.ts": { added: 3, removed: 0 } }), executor, planner)
  ...
  return (async () => {
    ...
    const teardownExecutor = {
      ...
      sendMessage: async () => {
        ...
        if (turnCount === 2) {
          channel.intents.push({ kind: "report-accepted", status: "failed", summary: "after teardown" })
        }
        return { info: { id: `m${turnCount}`, sessionID: "s", tokens: turnCount === 1 ? { input: 1_400_000, output: 0 } : {} }, parts: [] }
      },
      listMessages: async () => [],
      deleteSession: async () => {},
    }
    ...
    equal(result.outcome.status, "failed")
    const journal = store.readJournal(RUN_ID)
    const hasTeardown = journal.filter((e) => "phase" in e && e.phase === "teardown_demanded").length
  ...
  return (async () => {
    ...
    const redemandExecutor = {
      ...
      sendMessage: async () => {
        ...
        channel.intents.push({ kind: "report-accepted", status: "failed", summary: "after re-demand" })
        return { info: { id: "m3", sessionID: "s", tokens: {} }, parts: [] }
      },
      listMessages: async () => [],
      deleteSession: async () => {},
    }
    const config = Config.parse({ thresholds: { rotationFraction: 0.01, ladderRotateAt: 3, ladderParkAt: 5 } })
    ...
    ok(journal.some((e) => "phase" in e && e.phase === "teardown_demanded"), "should have teardown_demanded rotation phase")
